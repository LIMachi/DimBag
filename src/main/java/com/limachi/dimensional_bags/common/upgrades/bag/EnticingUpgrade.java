...
public class EnticingUpgrade extends BaseUpgradeBag<EnticingUpgrade> {
    ...
    static {
        BagUpgradeManager.registerUpgrade(NAME, EnticingUpgrade::new);
    }

    @Override
    public ActionResultType upgradeEntityTick(int bagId, World world, Entity entity) {
        if (entity instanceof BagEntityItem && !world.isClientSide && EnticingUpgrade.getInstance(EnticingUpgrade.NAME).isActive(bagId)) {
