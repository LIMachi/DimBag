...
public class TimeDistortionUpgrade extends BaseUpgradeBag<TimeDistortionUpgrade> {
    ...
    static {
        BagUpgradeManager.registerUpgrade(NAME, TimeDistortionUpgrade::new);
    }
    ...
    public static boolean filterTE(int eye, TileEntity te) {
        return !(te instanceof IisBagTE || te instanceof BeaconTileEntity || te instanceof ConduitTileEntity || te instanceof ProxyTileEntity) && (te.getBlockPos().getX() - SubRoomsManager.ROOM_OFFSET_X + SubRoomsManager.HALF_ROOM) / SubRoomsManager.ROOM_SPACING + 1 == eye && isValidByList(te.getType().getRegistryName(), BLACK_LIST_TE, false, false);
    }
    ...
    @Override
    public ActionResultType upgradeEntityTick(int bagId, World worldIn, Entity entity) {
        ...
        if (!upgrade.isActive(bagId)) return ActionResultType.PASS;
        int factor = upgrade.getSetting(bagId, "factor");
        if (factor < 0) {
            removeTickingTEInBag(bagId, world);
            forEachEntityInBag(bagId, world, e->e.canUpdate(false));
        }
        else {
            addTickingTEInBag(bagId, world);
            forEachEntityInBag(bagId, world, e->e.canUpdate(true));
        }
        if (factor >= 0 || EventManager.tick % Math.pow(2, -factor) == 0) {
            final int mi = factor > 0 ? (int)Math.pow(2, factor) : 1;
            iterTickingTEInBag(bagId, world).forEach(te->{
                for (int i = 0; i < mi; ++i) {
                    if (factor >= 0 && (i == 0 || EnergyData.extractEnergy(bagId, ENERGY_CONSUMPTION) != ENERGY_CONSUMPTION)) continue; //only consume energy if the factor is positive, and skip the first tick (has it will/was handled by the vanilla tick)
                    ((ITickableTileEntity) te).tick();
                }
            });
            forEachEntityInBag(bagId, world, e->{
                for (int i = 0; i < mi; ++i) {
                    if (factor >= 0 && (i == 0 || EnergyData.extractEnergy(bagId, ENERGY_CONSUMPTION) != ENERGY_CONSUMPTION)) continue; //only consume energy if the factor is positive, and skip the first tick (has it will/was handled by the vanilla tick)
                    e.tick();
                }
            ...
            if (randomTick > 0 && world instanceof ServerWorld)
                randomTickBagBlocks(bagId, (ServerWorld)world, randomTick * (mi - 1) * 4);
        } else {
            int[] ec = {0};
            forEachEntityInBag(bagId, world, e->++ec[0]);
            EnergyData.receiveEnergy(bagId, (int) ((Math.pow(2, -factor) - 1) * ENERGY_PRODUCTION * (iterTickingTEInBag(bagId, world).count() + ec[0])));
        }
