...
public abstract class BaseUpgradeBag<T extends BaseUpgradeBag<?>> extends Item {
...
//    public static <T> T getSetting(int eye, String name, String label, T def) { return SettingsData.execute(eye, sd->UpgradeManager.getUpgrade(name).settings.get(label, sd), def); }
    ...
    public boolean isInstalled(int bagId) { return BagUpgradeManager.execute(bagId, um->um.getInstalledUpgrades().contains(upgradeName()), false); }

    public boolean isActive(int bagId) { return canBeInstalled() && isInstalled(bagId) && SettingsData.execute(bagId, sd->settingsReader.getOrDefault("active", sd, true), true); }

    public static <T extends BaseUpgradeBag> T getInstance(String name) { return (T) BagUpgradeManager.getUpgrade(name); }
    ...
    @OnlyIn(Dist.CLIENT)
    public <T extends LivingEntity> void onRenderEquippedBag(int bagId, BipedModel<T> entityModel, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {}

    @OnlyIn(Dist.CLIENT)
    public void onRenderBagEntity(int bagId, BagEntity entity, float yaw, float partialTicks, MatrixStack matrix, IRenderTypeBuffer buffer, int packedLight) {}

    public ActionResultType upgradeEntityTick(int bagId, World world, Entity entity) { return ActionResultType.PASS; } //called every X ticks by the bag manager

    private BaseUpgradeBag() {
        super(DimBag.DEFAULT_PROPERTIES);
    }

    protected BaseUpgradeBag(Item.Properties props) {
        super(props.stacksTo(new BaseUpgradeBag<T>() {
            @Override
    ...
    public CompoundNBT getMemory(BagUpgradeManager manager) { return manager.getMemory(getMemoryKey(), false); }
    public int getCount(BagUpgradeManager manager) { return manager.getUpgradeCount(upgradeName()); }

    public void addCount(BagUpgradeManager manager, int count) {
        CompoundNBT nbt = manager.getMemory(getMemoryKey(), true);
        int pc = nbt.getInt(BagUpgradeManager.COUNT_NBT_KEY);
        nbt.putInt(BagUpgradeManager.COUNT_NBT_KEY, pc + count);
        manager.setDirty();
    }
    ...
    public static void installUpgradeOnBagEntity(PlayerInteractEvent.EntityInteract event) {
        if (event.getItemStack().getItem() instanceof BaseUpgradeBag && event.getItemStack().getCount() > 0 && event.getTarget() instanceof BagEntity) {
            event.setCanceled(true);
            event.setCancellationResult(ActionResultType.SUCCESS);
            if (DimBag.isServer(event.getWorld()))
                event.getPlayer().setItemInHand(event.getHand(), installUpgrades(event.getPlayer(), ((BagEntity) event.getTarget()).getbagId(), event.getItemStack()));
        }
    }
    ...
    public @Nonnull ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
        if (DimBag.isServer(worldIn) && !(playerIn instanceof FakePlayer) && KeyMapController.KeyBindings.SNEAK_KEY.getState(playerIn)) {
            ...
            if (stack.getItem() instanceof BaseUpgradeBag && stack.getCount() > 0) {
                int bagId = SubRoomsManager.getbagId(worldIn, playerIn.blockPosition(), false);
                if (bagId == 0)
                    bagId = BagItem.getBag(playerIn, 0, true, false);
                if (bagId > 0)
                    return ActionResult.success(installUpgrades(playerIn, bagId, stack));
            }
        }
    ...
    public @Nonnull ActionResultType useOn(@Nonnull ItemUseContext context) {
        ...
        if (DimBag.isServer(context.getLevel()) && player != null && !(player instanceof FakePlayer) && KeyMapController.KeyBindings.SNEAK_KEY.getState(player)) {
            ...
            if (stack.getItem() instanceof BaseUpgradeBag && stack.getCount() > 0) {
                ...
                int bagId = te instanceof ProxyTileEntity ? ((ProxyTileEntity)te).bagId() : SubRoomsManager.getbagId(player.level, player.blockPosition(), false);
                if (bagId == 0)
                    bagId = BagItem.getBag(player, 0, true, false);
                if (bagId > 0) {
                    player.setItemInHand(context.getHand(), installUpgrades(player, bagId, stack));
    ...
    private static ItemStack installUpgrades(PlayerEntity player, int bagId, ItemStack stack) {
        BaseUpgradeBag upgrade = (BaseUpgradeBag) stack.getItem();
        ...
        BagUpgradeManager um = BagUpgradeManager.getInstance(bagId);
        ItemStack out = stack.copy();
        if (um.getMemory(upgrade.getMemoryKey(), true).getInt(BagUpgradeManager.COUNT_NBT_KEY) < upgrade.getMaxCount()) {
            int nc = ((BaseUpgradeBag) stack.getItem()).installUpgrade(um, stack.getCount());
            if (nc > 0) {
                out.shrink(nc);
    ...
    public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (!((BaseUpgradeBag) stack.getItem()).canBeInstalled())
     ...
     */
    public int installUpgrade(BagUpgradeManager manager, int qty) { return qty; }

    /**
     * if this upgrade is installed and active, this function will be called every X ticks (by default every tick)
     * @param bagId which bag is trying to print information on hud
     * @param player which player is using the bag and should receive a hud update
     * @param window
     * @param matrixStack
     * @param partialTicks
     */
    public void onRenderHud(int bagId, PlayerEntity player, MainWindow window, MatrixStack matrixStack, float partialTicks) {}
    ...
    public static void onRenderGameOverlay(RenderGameOverlayEvent.Pre event) { //might be used at some point to add hud ellements FIXME
        if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
            ...
            int bag = BagItem.getBag(player, 0, false, false);
            if (bag > 0) {
