...
@StaticInit
public class WorldAccessBlockTileEntity extends BaseTileEntity implements IEnergyStorage, IItemHandler, IFluidHandler, IisBagTE {

    public static final String NAME = "world_access";

    static {
        Registries.registerTileEntity(NAME, WorldAccessBlockTileEntity::new, ()->Registries.getBlock(WorldAccessBlock.NAME), null);
    }
    ...
    public WorldAccessBlockTileEntity() { super(Registries.getBlockEntityType(NAME)); }
    ...
    public void tick(int tick) {
        ...
        if (eye > 0) {
            Entity e = HolderData.getInstance(SubRoomsManager.getbagId(level, worldPosition, false)).getEntity();
            if (e == null) return;
            lastKnownHolder = new WeakReference<>(e);
            isEquipped = !(e instanceof BagEntity);

            if (isEquipped) {
                List<ItemEntity> tl = e.level.getEntitiesOfClass(ItemEntity.class, new AxisAlignedBB(e.blockPosition().offset(-RADIUS_EQUIPPED, -RADIUS_EQUIPPED, -RADIUS_EQUIPPED), e.blockPosition().offset(RADIUS_EQUIPPED, RADIUS_EQUIPPED, RADIUS_EQUIPPED)), f -> BagItem.getbagId(f.getItem()) != eye);
                worldItems = new ArrayList<>();
                for (ItemEntity ie : tl)
                    worldItems.add(new WeakReference<>(ie));
                inv = new WeakReference<>(null);
            } else {
                List<ItemEntity> tl = e.level.getEntitiesOfClass(ItemEntity.class, new AxisAlignedBB(e.blockPosition().offset(-DEFAULT_RADIUS, -DEFAULT_RADIUS, -DEFAULT_RADIUS), e.blockPosition().offset(DEFAULT_RADIUS, DEFAULT_RADIUS, DEFAULT_RADIUS)), f -> BagItem.getbagId(f.getItem()) != eye);
                worldItems = new ArrayList<>();
                for (ItemEntity ie : tl)
                    worldItems.add(new WeakReference<>(ie));
            ...
            if (WorldAccessBlock.isPowered(getBlockState()) && cooldown == 0 && !holdItem.isEmpty() && e instanceof LivingEntity)
    ...
    public void simulateRightClick(LivingEntity e, ItemStack holdItem) {
        if (extractEnergy(ENERGY_USAGE_PER_ACTION, true) == ENERGY_USAGE_PER_ACTION) {
            ...
            if (e instanceof PlayerEntity) {
                ...
                BlockRayTraceResult rr = BagItem.rayTrace(e.level, (PlayerEntity) e, RayTraceContext.FluidMode.NONE);
