package com.limachi.dimensional_bags.common.bagDimensionOnly.bagPerformer.performances;

import com.limachi.dimensional_bags.lib.utils.ClassUtils;
...
public class EntityPerformer {
    protected static class ExecutorMao<O extends Entity> {
        ...
        public boolean run(String command, int bagId, Object entity) {
            if (entity instanceof Entity && clazz.isInstance(entity) && map.containsKey(ClassUtils.Strings.splitIgnoringQuotes(command, " ", 0)[0])) {
                String[] s = ClassUtils.Strings.splitIgnoringQuotes(command, " ", 0);
                Entry e = map.get(s[0]);
                ...
                for (int i = 0; i < pt.length; ++i)
                    if ((params[i] = ClassUtils.parse(pt[i], s[1 + i])) == null)
                        return false;
                if (EnergyData.execute(bagId, energyData -> energyData.extractEnergy(e.energyCost, true) == e.energyCost, false)) {
                    EnergyData.execute(bagId, energyData -> energyData.extractEnergy(e.energyCost, false));
    ...
    public <T extends Entity> EntityPerformer(T entity, int bagId) {
        this.entity = entity;
        this.bagId = bagId;
    }
    ...
    public void run(String command) {
        if (isValidConsumer(command)) {
            for (Map.Entry<Class<? extends Entity>, ExecutorMao<?>> entry : cache.entrySet())
                if (entry.getValue().run(command, bagId, entity))
                    return;
        }
    }
}
