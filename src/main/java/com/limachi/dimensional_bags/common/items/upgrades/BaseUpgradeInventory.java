...
import com.limachi.dimensional_bags.common.tileentities.IInstallUpgradeTE;
    ...
    ActionResultType useOn(@Nonnull ItemUseContext context) {
        ...
        if (DimBag.isServer(context.getLevel()) && player != null && !(player instanceof FakePlayer) && KeyMapController.KeyBindings.SNEAK_KEY.getState(player)) {
            ...
            if (stack.getItem() instanceof BaseUpgradeInventory && stack.getCount() > 0) {
                ...
                if ((te instanceof PillarTileEntity && target == UpgradeTarget.PILLAR)
                        || (te instanceof FountainTileEntity && target == UpgradeTarget.FOUNTAIN)
                        || (te instanceof CrystalTileEntity && target == UpgradeTarget.CRYSTAL)) {
