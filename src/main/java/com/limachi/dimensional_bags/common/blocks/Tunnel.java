...
public class Tunnel extends Block {
    ...
    public Tunnel() { super(Properties.of(Material.HEAVY_METAL).strength(-1f, 3600000f).sound(SoundType.GLASS)); }
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult ray) { //'right-click' behavior (use/interact)
        ...
        if (KeyMapController.KeyBindings.SNEAK_KEY.getState(player))
            SubRoomsManager.execute(SubRoomsManager.getEyeId(world, pos, false), subRoomsManager -> subRoomsManager.enterBag(player, false, false, false, false, false));
        else
