...
public abstract class TextField<C extends BaseContainer<C>> extends BaseContainerWidget<C> {
    ...
    protected Consumer<TextField> onFinishInput;
    ...
    public TextField(C handler, double x, double y, double width, double height, @Nullable String initialText, @Nullable BiFunction<String, String, Boolean> validateText, @Nullable Consumer<TextField> onFinishInput) {
        super(handler, new Box2d(x, y, width, height), true);
        if (initialText != null)
            setText(initialText);
        pushToHistory();
    ...
    public FontRenderer getFont() {
        if (font == null)
            font = screen(SimpleContainerScreen::getFont, null);
        return font;
    }
    ...
    private void moveCursorToMouse(double mouseX, double mouseY) {
        ...
        while (i <= s.length() && lx + RenderUtils.getPrintedStringWidth(getLocalMatrix(), getFont(), s.substring(0, i)) < mouseX)
            ++i;
    ...
    ClientEvents clientEvents() {
        return new ClientEventsDefault() {
