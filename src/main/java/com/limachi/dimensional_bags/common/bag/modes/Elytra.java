...
public class Elytra {} /*extends Mode {
    ...
    @Override
    public ActionResultType onEntityTick(int bagId, World world, Entity entity, boolean isSelected) {
    ...
    @Override
    public ActionResultType onItemUse(int bagId, World world, PlayerEntity player, BlockRayTraceResult ray) { return ActionResultType.SUCCESS; }
    ...
    public ActionResultType onActivateItem(int bagId, PlayerEntity player) {
        ...
        if (UpgradeManager.execute(bagId, upgradeManager -> upgradeManager.getInstalledUpgrades().contains("upgrade_energy"), false)) {
            EnergyData energyData = EnergyData.getInstance(bagId);
    ...
    public void onRenderHud(int bagId, boolean isSelected, PlayerEntity player, MainWindow window, MatrixStack matrixStack, float partialTicks) {
        if (isSelected) {
            if (UpgradeManager.execute(bagId, upgradeManager -> !upgradeManager.getInstalledUpgrades().contains("upgrade_energy"), true)) {
                IDimBagCommonItem.ItemSearchResult res = IDimBagCommonItem.searchItem(player, 10, FireworkRocketItem.class, (x) -> {
                    CompoundNBT nbt = x.getChildTag("Fireworks");
                    return nbt == null || nbt.getList("Explosions", 10).size() == 0;
                }, true);
