...
import com.limachi.dimensional_bags.lib.common.fluids.BinaryStateSingleFluidHandler;
...
public class Tank extends AbstractMode {
    ...
    public ActionResultType onItemRightClick(int bagId, World world, PlayerEntity player) {
        TankData mt = TankData.getInstance(bagId);
        ...
        for (RayTraceContext.FluidMode mode : new RayTraceContext.FluidMode[]{RayTraceContext.FluidMode.SOURCE_ONLY, RayTraceContext.FluidMode.NONE}) {
            RayTraceResult ray = BagItem.rayTrace(world, player, mode);
            if (ray.getType() == RayTraceResult.Type.BLOCK) { //we found a source block, first we try to take it, if we can't and the tank is of different fluid type, try to replace the fluid block
                ...
                if (world.mayInteract(player, blockHit) && player.mayUseItemAt(blockOffset, blockraytraceresult.getDirection(), new ItemStack(new BagItem()))) {
        ...
        TankContainer.open(player, bagId, null);
        return ActionResultType.SUCCESS;
    }
}