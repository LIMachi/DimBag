...
import com.limachi.dimensional_bags.lib.common.network.packets.WorldSavedDataSyncMsg;
...
public class WorldSavedDataManager {
    ...
    public abstract static class EyeWorldSavedData extends WorldSavedData {
        ...
        @Override
        public void setDirty() { //FIXME: setDirty is spammed when the entity using the bag moves, when an inventory is opened (only client side, du to local inventory slots overriden by server), or when a client change the bag mode
            super.setDirty();
        ...
        public WorldSavedDataSyncMsg preparePacket(boolean send_all) {
            ...
            if (!send_all) {
                ...
                if (diff.toString().length() < prevState.toString().length())
                    out = new WorldSavedDataSyncMsg(getSuffix(), getbagId(), diff, true);
                else
                    out = new WorldSavedDataSyncMsg(getSuffix(), getbagId(), ser, false);
            }
            else
                out = new WorldSavedDataSyncMsg(getSuffix(), getbagId(), ser, false);
            prevState = ser;
