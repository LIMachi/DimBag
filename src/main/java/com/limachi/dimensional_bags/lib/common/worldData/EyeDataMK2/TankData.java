...
import com.limachi.dimensional_bags.lib.common.fluids.EmptySimpleFluidHandlerSerializable;
...
public class TankData extends WorldSavedDataManager.EyeWorldSavedData implements ISimpleFluidHandlerSerializable {
    ...
    protected final ArrayList<TankInventory> fountains = new ArrayList<>();
    ...
    public void addFountain(TankInventory tank) {
    ...
    public void removeFountain(UUID id) {
        TankInventory font = (TankInventory)getFountainTank(id);
    ...
    public ISimpleFluidHandlerSerializable getFountainTank(@Nullable UUID id) {
        ...
        for (TankInventory inv : fountains)
    ...
    public void readFromBuff(PacketBuffer buff) {
        ...
        List<UUID> toInvalidate = fountains.stream().map(TankInventory::getId).collect(Collectors.toList());
        int np = buff.readInt();
        int no = buff.readInt();
    ...
    public void writeToBuff(PacketBuffer buff) {
        ...
        for (UUID id : fountainsOrder)
            buff.writeUUID(id);
        for (TankInventory tank : fountains)
