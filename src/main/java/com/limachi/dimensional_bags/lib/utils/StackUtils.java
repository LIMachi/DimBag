...
public class StackUtils {
    ...
    public static int playerSlot(EquipmentSlotType slot) { return slot.getType() == EquipmentSlotType.Group.HAND ? slot.getIndex() == 0 ? 0 : 40 : slot.getIndex() + 36; }

    /**
     * simulate ticking of items on a player (will use the first non selected/off hand/armor slot if target slot is null), return a list of items
     * @param items
     * @param player
     * @param targetSlot
     * @return
     */
    public static ArrayList<ItemStack> tickOnPlayer(ArrayList<ItemStack> items, PlayerEntity player, @Nullable EquipmentSlotType targetSlot) {
        if (items == null)
            return new ArrayList<>();
        if (player == null || items.isEmpty())
            return items;
        Supplier<ItemStack> getter = targetSlot == null ? player.inventory.selected == 0 ? ()->player.inventory.items.get(1) : ()->player.inventory.items.get(0) : ()->player.getItemBySlot(targetSlot);
        Consumer<ItemStack> setter = targetSlot == null ? player.inventory.selected == 0 ? s->player.inventory.items.set(1, s) : s->player.inventory.items.set(0, s) : s->player.setItemSlot(targetSlot, s);
        int slot = targetSlot == null ? player.inventory.selected == 0 ? 1 : 0 : playerSlot(targetSlot);
        boolean selected = targetSlot != null && targetSlot.getType() == EquipmentSlotType.Group.HAND;
        ItemStack original = getter.get();
        ArrayList<ItemStack> out = new ArrayList<>();
        for (ItemStack t: items) {
            setter.accept(t);
            t.inventoryTick(player.level, player, slot, selected);
            out.add(getter.get());
        }
    ...
    public static ItemStack tickOnPlayer(ItemStack item, PlayerEntity player, @Nullable EquipmentSlotType targetSlot) {
        if (item == null || item.isEmpty() || player == null) return ItemStack.EMPTY;
        Supplier<ItemStack> getter = targetSlot == null ? player.inventory.selected == 0 ? ()->player.inventory.items.get(1) : ()->player.inventory.items.get(0) : ()->player.getItemBySlot(targetSlot);
        Consumer<ItemStack> setter = targetSlot == null ? player.inventory.selected == 0 ? s->player.inventory.items.set(1, s) : s->player.inventory.items.set(0, s) : s->player.setItemSlot(targetSlot, s);
        ItemStack original = getter.get();
        setter.accept(item);
        item.inventoryTick(player.level, player, targetSlot == null ? player.inventory.selected == 0 ? 1 : 0 : playerSlot(targetSlot), targetSlot != null && targetSlot.getType() == EquipmentSlotType.Group.HAND);
        ItemStack out = getter.get();
        setter.accept(original);
        return out;
    }
