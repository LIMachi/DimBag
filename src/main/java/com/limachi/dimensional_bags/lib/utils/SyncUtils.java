...
public class SyncUtils {
    ...
    public static void resyncPlayerSlot(@Nonnull ServerPlayerEntity player, int slot) {
        if (slot >= 0 && slot < 41)
            player.connection.send(new SSetSlotPacket(player.containerMenu.containerId, slot, player.containerMenu.getSlot(slot).getItem()));
    }
    ...
    public static class XPSnapShot {
        ...
        public XPSnapShot(PlayerEntity player) {
