package com.limachi.dimensional_bags.lib;
...
public class CuriosIntegration {
    ...
    /**
     * only find valid bags (bagId > 0) in the CuriosIntegration.BAG_CURIOS_SLOT slots of the living entity
     */
    public static List<ProxySlotModifier> getEquippedBags(LivingEntity entity) {
        ...
        for (int i = 0; i < sh.getSlots(); ++i)
            if (sh.getStackInSlot(i).getItem() instanceof BagItem && BagItem.getbagId(sh.getStackInSlot(i)) > 0) {
                int finalI = i;
