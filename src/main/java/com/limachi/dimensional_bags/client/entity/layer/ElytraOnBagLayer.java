...
@OnlyIn(Dist.CLIENT)
public class ElytraOnBagLayer<T extends LivingEntity, M extends EntityModel<T>> extends LayerRenderer<T, M> {
    private static final ResourceLocation TEXTURE_ELYTRA = new ResourceLocation("textures/entity/elytra.png");
    private final ElytraModel<T> modelElytra = new ElytraModel<>();

    public ElytraOnBagLayer(IEntityRenderer<T, M> rendererIn) {
        super(rendererIn);
    }
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (shouldRender(itemstack, entitylivingbaseIn)) {
            ...
            matrixStackIn.translate(0.0D, -0.11D, 0.44D);
...
//        return stack.getItem() == Items.ELYTRA;
        return stack.getItem() instanceof Bag && stack.getTag() != null && stack.getTag().getBoolean("ElytraAttached");
    }
